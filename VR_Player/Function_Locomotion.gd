...
enum ROTATION_TYPE { NONE, SMOOTH, SNAP, STRAFE }
enum LOCOMOTION_TYPE { NONE, SMOOTH, SMOOTH_AND_STRAFE}
...
export (ROTATION_TYPE) var rotation_type = ROTATION_TYPE.SNAP
export (LOCOMOTION_TYPE) var move_type = LOCOMOTION_TYPE.SMOOTH
...
func _physics_process(delta):
	...
	if controller.get_is_active():
		...
		# move_type == MOVEMENT_TYPE.move_and_strafe
			...
			if(rotation_type == ROTATION_TYPE.SMOOTH && abs(left_right) > 0.1):
				...
				turn_step = 0.0
			elif(rotation_type == ROTATION_TYPE.SNAP && abs(left_right) > 0.1):
					if left_right > 0.0:
						if turn_step < 0.0:
							# reset step
							turn_step = 0
			...
			if move_type != LOCOMOTION_TYPE.NONE:
				var curr_transform = $KinematicBody.global_transform
